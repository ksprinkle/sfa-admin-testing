...
| Render's production PostgreSQL database (`sfa-db`) was never tracked by Alembic — schema was built entirely by `api/main.py`'s unconditional `Base.metadata.create_all()` across many boots | `api/main.py` → `ARCHITECTURE_OVERVIEW.md` § Backend Architecture, `CLAUDE.md` § Development Guardrails | Open — `feedback` table resolved 2026-07-16 (columns added by hand, `alembic_version` stamped to `7f6c9df3d7fd`); a full table-by-table audit against current models is still pending | Discovered via `GET /api/feedback` returning an unhandled `500` in production. `alembic_version` had never been stamped at all; running `alembic upgrade head` from scratch replayed the full migration history and failed immediately on an unrelated, much older migration (`4340172195f0_add_notes_column_to_participants`, "column already exists") because `create_all()` had already applied that change out-of-band. Because `create_all()` never retroactively alters existing tables, **any table created before a later column was added to its model is likely still missing that column in production**, independent of what `feedback` needed — the same audit script (comparing `Base.metadata` against `sqlalchemy.inspect(engine)` output) should be run against every table, not just the one that happened to surface first. Deploying to Render does not run migrations automatically; any future migration needs a manual `alembic upgrade head` (or, where drift is hit, a hand-applied targeted fix + `alembic stamp`) via a Render Shell session on `sfa-api`. |
