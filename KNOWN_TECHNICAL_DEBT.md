...
| Render's production PostgreSQL database (`sfa-db`) is behind the Alembic head — missing at least revision `7f6c9df3d7fd_add_feedback_submitter_attribution` | `alembic/versions/7f6c9df3d7fd_add_feedback_submitter_attribution.py`; `render.yaml` (`buildCommand`/`startCommand` run no migration step) | Resolved 2026-07-16 for the `feedback` table (columns added by hand and `alembic_version` stamped to `7f6c9df3d7fd`); mechanism still open | Confirmed by direct probe: `GET /api/feedback` returned an unhandled `500` (plain-text "Internal Server Error", not a FastAPI JSON error) against the deployed Render backend. Root cause: production's schema was built entirely by `api/main.py`'s unconditional `Base.metadata.create_all()` across many boots, never by Alembic — `alembic_version` had never been stamped at all. Running `alembic upgrade head` from scratch replayed the full migration history and failed immediately on an unrelated, much older migration (`4340172195f0_add_notes_column_to_participants`, "column already exists") because `create_all()` had already applied that change out-of-band. **This means other tables may have similar undetected drift** — a full schema audit (comparing live Postgres columns against current models table-by-table) is still worth doing separately. Deploying to Render does not run migrations automatically; any future migration still needs a manual `alembic upgrade head` (or, if drift is hit again, a hand-applied targeted fix + `alembic stamp`) via a Render Shell session on `sfa-api`. |
| Every `npm run build` of `admin-app` produces a hashed `docs/assets/index-*.js`/`.css` filename that is silently excluded by `.gitignore` (`docs/assets/index-*.css`, `docs/assets/index-*.js`) | `.gitignore`; `admin-app/vite.config.js` (`assetFileNames`/`entryFileNames` use `index-[hash]`) | Open | Because the hash changes on every build, the new bundle is always untracked and ignored — a plain `git add`/`git commit` after a rebuild silently ships an `index.html` that references a JS/CSS file not present in the commit, breaking the live GitHub Pages site. History shows this has previously been worked around with `git add -f` on the specific hashed filename (e.g. commit `7913ea4`) rather than fixed at the `.gitignore` level. Anyone rebuilding `docs/` must `git add -f` the new `docs/assets/index-*.js` and `.css` files explicitly, or adjust `.gitignore`/`entryFileNames` so this stops being a manual step. |
