...
| Render's production PostgreSQL database (`sfa-db`) is behind the Alembic head — missing at least revision `7f6c9df3d7fd_add_feedback_submitter_attribution` | `alembic/versions/7f6c9df3d7fd_add_feedback_submitter_attribution.py`; `render.yaml` (`buildCommand`/`startCommand` run no migration step) | Open | Confirmed by direct probe: `GET /api/feedback` returns an unhandled `500` (plain-text "Internal Server Error", not a FastAPI JSON error) against the deployed Render backend, while the identical query succeeds locally against a database migrated to head. The `Feedback` model's `submitted_by_user_id`/`submitted_by_name`/`submitted_by_email` columns (added by `7f6c9df3d7fd`) most likely don't exist on the production table yet. **Required fix:** run `alembic upgrade head` against the production `DATABASE_URL` (e.g. via a Render Shell session on the `sfa-api` service) to bring `sfa-db` to revision `7f6c9df3d7fd`, then confirm with `alembic current`. Deploying to Render does not run migrations automatically — this must be repeated manually after any future migration lands. |
