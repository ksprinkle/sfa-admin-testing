...
### Confirmed production limitation of the legacy ownership model (discovered 2026-07-25, during B13b validation)

A parent registers a child under the child's own email (`register_public_participant()`, `api/services/public_registration.py`); the resulting `Participant` row gets `user_id` = the parent **and** `person_id` = the **parent's own `Person`** (looked up solely by `current_user.id` — never the child's submitted name/email). No `Person` is ever created for the child at registration time. When the child later creates their own account (`POST /auth/register`), a brand-new `Person` is created unconditionally, with no lookup against any existing `Person`/`Participant` by email first. The result: the child can register, log in, and verify their email, and still never sees their own registration — `claim_participants_for_user()`'s Pass 1 excludes the row (its `user_id` is already non-null, set to the parent), and Pass 2 can't help either (`person_id` names the parent, not the child, so no relationship check lines up). This is **not** a B13a/B13b regression — both are exactly as scoped, and this defect predates them (Slice B7). It also does not violate B12's reconciliation invariant (`person_id`/`user_id` "identify the same person" for every row) — both fields consistently name the *parent*, just not the *correct* subject. **Critically, B13c/B13d as scoped will not fix this**: their entire job is proving the new resolution mechanism reproduces today's behavior exactly before cutover — by design, they will faithfully carry this exact bug forward, not fix it. Root cause traced directly to Phase 3A's own design never having been fully implemented — `PHASE3A_UNIFIED_IDENTITY_AND_HOUSEHOLD_ARCHITECTURE_REVIEW.md` §2.2 specified both "a `Person` is created for the child with no `User` attached" (registration-by-someone-else) and "attach the `User` to the existing `Person` rather than creating a duplicate" (later self-claiming) — neither was built this way. Full detail, including why no new schema or architecture is required to fix it, in [`PHASE3C_SLICE_B14_IDENTITY_CLAIM_PERSON_LINKING_ARCHITECTURE_REVIEW.md`](PHASE3C_SLICE_B14_IDENTITY_CLAIM_PERSON_LINKING_ARCHITECTURE_REVIEW.md).

### B14 — Identity Claim & Person Linking (proposed, review only — not authorized)

**Architecture review completed 2026-07-25** (`PHASE3C_SLICE_B14_IDENTITY_CLAIM_PERSON_LINKING_ARCHITECTURE_REVIEW.md`): proposes fixing the two write paths responsible for the limitation above — (1) `register_public_participant()` should create/reuse a headless `Person` for the actual subject when registering on someone else's behalf, instead of stamping the registrant's own `person_id`, and express the registrant's authority via `PersonRelationship` rather than `participant.user_id`; (2) `/auth/register` should check for an existing headless `Person` by email before creating a new one, attaching the new `User` to it instead of creating a disconnected duplicate. Confirms this needs **no new schema or architecture** — `Person.user_id` being nullable already supports a headless `Person`, and `PersonRelationship` already expresses delegated, multi-manager authority; this is a service-layer write-path correctness slice. Sequenced strictly **after B13d** (fixing the write path is moot before something reads through `person_id`+relationships for real) and deliberately **not** folded into B13d itself (which is a behavior-preserving cutover — bundling a correctness fix into it would break the one property B13d exists to prove). Four open decisions flagged for explicit authorization before this slice starts: whether to allow narrowly-scoped self-service `PersonRelationship` creation at registration time (expanding B13a's admin-only scope), whether to backfill already-affected production rows or fix prospectively only, whether to add a partial unique constraint on `Person.email`, and whether to adopt this minimal fix or Phase 3A's fuller `person_id`/`registered_by_person_id` column split. **Not authorized. Depends on B13c and B13d completing first.**

### Future — Retirement of `User.role` / `Participant.user_id` (explicitly future, not part of this rollout)

`User.role` retirement remains gated on B11's continuous-issuance model (already shipped) proving out over a burn-in period. `Participant.user_id` retirement is gated on the new B13 (Relationship-Aware Ownership Resolution) existing and being proven equivalent to today's `user_id`-based ownership, **and now also on B14 (Identity Claim & Person Linking)** — retiring `user_id` while `person_id` can still be silently mis-stamped to the wrong subject (the limitation discovered 2026-07-25, above) would remove the one fallback signal these affected rows have, with nothing correct to replace it. Not on a fixed timeline, per the decision above. A dedicated future slice would then stop writing the legacy fields and, eventually, drop them — with its own guarded migration and its own call-site audit, repeating B0's discipline rather than assuming the original audit still holds. Explicitly out of scope for this roadmap; listed only so it isn't lost.
