...
**Complete, pending your review before deployment** — see [`PHASE3B_SLICE_B6_SHADOW_VALIDATION_REPORT.md`](PHASE3B_SLICE_B6_SHADOW_VALIDATION_REPORT.md). Shadow-check added to `GET /api/participants/mine` only; `api/services/capability_resolution.py` gained an additive `resolve_capabilities_with_context()` (re-verified against all of B5's equivalence tests, unchanged). Proven directly (not by inference): a forced-mismatch test shows the response is unaffected either way, a spy confirms the engine runs on every eligible request, and the full existing suite (129 tests, including 13 pre-existing tests against this exact endpoint) shows zero spurious mismatch warnings. `has_permission()` and every router dependency/decorator are byte-for-byte unchanged. Not yet deployed.
