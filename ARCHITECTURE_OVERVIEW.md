...
**Onboarding orchestration:** since 2026-07-18, the canonical public registration route (`POST /api/public/events/{slug}/register`) continues a successful registration straight into waiver issuance when a waiver is required — a second example of the same layering, at the orchestration level rather than within a single service. `api/services/public_onboarding.py::complete_public_registration()` contains no registration or waiver logic itself; it calls `register_public_participant()` (`api/services/public_registration.py`, unchanged) and, if `waiver_template_lifecycle.get_active_template()` returns a template, `create_signing_token()` (`api/services/waiver_signing.py`, unchanged — the same function an admin's manual "create waiver signing token" action already uses) with `actor_user_id=None` to mark it system-issued rather than admin-issued. "Waiver required" here means "an active `WaiverTemplate` exists" — there's no separate per-event/per-role waiver-required flag in the data model. The legacy `/events/{slug}/participants` endpoint deliberately does not call this orchestration, preserving its response shape from the consolidation described above.
...
API access goes through a single `fetch`-based client (`admin-app/src/api/api.js`) with per-domain modules (`events.js`, `communications.js`, etc.) layered on top; a `401` response clears the session and redirects to login. `admin-app/src/api/baseUrl.js` resolves the API origin per environment and explicitly rejects private/loopback URLs in production builds. The portal's own `admin-app/src/api/portal.js` deliberately does not reuse this client — `apiFetch` attaches whatever token happens to be in `localStorage` and force-redirects to `/login` on a `401`, both wrong for a public, unauthenticated surface — and instead does plain unauthenticated `fetch` calls against the public event and registration endpoints (including the write path, `PortalRegister`'s `POST /api/public/events/{slug}/register`), reusing only `baseUrl.js`'s origin resolution. Where a portal page does need to know whether the visitor is authenticated (`PortalMyRegistrations`, calling the token-gated `GET /api/participants/mine`), session state is likewise isolated from the admin session: `admin-app/src/api/portalAuth.js` reads/writes/clears a separate `portal.token`/`portal.profile` `localStorage` pair rather than `auth.js`'s `token`/`auth.profile` keys, and dispatches its own `portal-auth:changed` window event (mirroring, but never sharing, `auth.js`'s `auth:changed`) so `PortalLayout` can reflect sign-in state without a full page reload. `PortalLogin.jsx` (since 2026-07-19) calls the same `POST /api/auth/login` endpoint the admin `Login.jsx` uses — no backend or role-specific auth change was needed, since login was already role-agnostic — a participant session can still never make the admin shell appear signed in, or vice versa. There is still no participant-facing account-creation flow anywhere, admin or portal, even though `POST /api/auth/register` already exists on the backend — see `KNOWN_TECHNICAL_DEBT.md`.
