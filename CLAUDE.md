...
3. **Schema changes require a real Alembic migration.** Do not rely on `api/main.py`'s unconditional `Base.metadata.create_all()` call for schema changes — any new table, column, or constraint needs a corresponding revision in `alembic/versions/`. See [`KNOWN_TECHNICAL_DEBT.md`](KNOWN_TECHNICAL_DEBT.md) for why this matters.
