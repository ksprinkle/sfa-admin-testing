...
**Email verification (backend infrastructure only, shipped 2026-07-18 — Hybrid Participant Account Model roadmap, Slice C):** `api/services/account_verification.py` (`create_verification_token()`, `verify_email_token()`) mirrors the waiver-signing token pattern (`api/services/waiver_signing.py`) against a new generic `user_action_tokens` table (`UserActionToken` — hashed, single-use, expiring, invalidate-on-reissue, with a `purpose` column so a future password-reset flow can reuse the same table). `POST /auth/register` now auto-issues and sends (best-effort — a delivery failure never fails registration) a verification token via the real provider-based email service (`api/services/email_delivery.py`, currently the no-op provider in production until SMTP is configured); `POST /auth/verify-email/resend` (rate-limited, does not leak whether an email is registered) and `POST /auth/verify-email/confirm` set `users.email_verified_at`. Deliberately excludes: claiming historical/unlinked `Participant` rows, any verification UI, and any change to login behavior (unverified users can still log in) — those are later slices (D/E).

| Layer | Location |
|---|---|
| Backend router | `api/routers/auth.py` |
| Backend service/support | `api/dependencies.py`, `api/security.py`, `api/services/authorization.py`, `api/services/rate_limiting.py`, `api/utils/email_normalization.py`, `api/services/account_verification.py`, `api/services/email_delivery.py` |
| Data model | `User` (incl. `email_verified_at`), `UserActionToken` (migration `e8b4a2f6c1d9`) |
...
**Account creation (`/portal/create-account`, shipped 2026-07-19 — Hybrid Participant Account Model roadmap, Slice B):** `PortalCreateAccount.jsx` reuses the Slice A-hardened `POST /auth/register` (via `api/portalAuth.js::createParticipantAccount()`) and immediately chains into the existing, unmodified `loginParticipant()` (`registerAndSignIn()`) — account creation, sign-in, and redirect to `/portal/my-registrations` in one flow, no new session logic. A real visitor can now obtain a participant account end-to-end for the first time. `POST /auth/register` now also auto-issues an email verification token as of Slice C (§1) — transparent to this flow, since verification doesn't gate login or registration. Deliberately excludes any verification UI and registration-claiming — both remain unbuilt, per the architecture review's roadmap (Slices D–E).
