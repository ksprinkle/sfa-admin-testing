...
| Layer | Location |
|---|---|
| Backend router | None new — reuses existing public endpoints (§2) read/write |
| Backend service | `api/services/public_onboarding.py` (new — orchestrates `public_registration.py` + `waiver_signing.py`, no waiver/registration logic of its own) |
