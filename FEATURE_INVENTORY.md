...
| Data model | `User` (incl. `email_verified_at`), `UserActionToken` (migration `e8b4a2f6c1d9`), `Participant.user_id` (claim target), `AdminAuditEvent` (claim audit trail, no schema change — `action` is a free-form column) |
...
**Account creation (`/portal/create-account`, shipped 2026-07-19 — Hybrid Participant Account Model roadmap, Slice B):** `PortalCreateAccount.jsx` reuses the Slice A-hardened `POST /auth/register` (via `api/portalAuth.js::createParticipantAccount()`) and immediately chains into the existing, unmodified `loginParticipant()` (`registerAndSignIn()`) — account creation, sign-in, and redirect to `/portal/my-registrations` in one flow, no new session logic. A real visitor can now obtain a participant account end-to-end for the first time. `POST /auth/register` now also auto-issues an email verification token as of Slice C, and verifying that token now automatically claims any matching historical registrations as of Slice D (§1) — both transparent to this flow, since neither gates login or registration. Deliberately excludes any verification/claiming UI — that's Slice E, the only piece of the architecture review's roadmap still unbuilt.
