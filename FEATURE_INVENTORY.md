...
**Feature Status:** Shipped. Two roles wired (`participant`, `admin`), each with its own least-privilege permission set — `admin` holds the operational `*.manage`/`admin.access` permissions, `participant` holds `participants.view_own` and `waivers.view_own` (Participant Portal identity foundation, added 2026-07-18; portal UI, including participant login and self-service account creation, shipped in the slices that followed — see §14). See `ARCHITECTURE_OVERVIEW.md`'s Auth & Authorization section for the underlying mechanism. `POST /auth/register` (account creation) was hardened 2026-07-19 — validated JSON body (`UserCreate`, reused as-is, previously unused), case-insensitive duplicate detection via the new shared `api/utils/email_normalization.py` helper, a minimum password length, and per-IP rate limiting (`api/services/rate_limiting.py`).

**Email verification (backend infrastructure only, shipped 2026-07-19 — Hybrid Participant Account Model roadmap, Slice C, baseline `v1.28.0`):** `api/services/account_verification.py` (`create_verification_token()`, `verify_email_token()`) mirrors the waiver-signing token pattern (`api/services/waiver_signing.py`) against a new generic `user_action_tokens` table (`UserActionToken` — hashed, single-use, expiring, invalidate-on-reissue, with a `purpose` column so a future password-reset flow can reuse the same table). `POST /auth/register` now auto-issues and sends (best-effort — a delivery failure never fails registration; a genuine SMTP failure is now logged rather than silently discarded) a verification token via the real provider-based email service (`api/services/email_delivery.py`); `POST /auth/verify-email/resend` (rate-limited, does not leak whether an email is registered, but now returns a real `502` if the provider itself fails to send — sending is that endpoint's only job) and `POST /auth/verify-email/confirm` set `users.email_verified_at`. Deliberately excludes any verification UI and any change to login behavior (unverified users can still log in) — that's a later slice (E). Shipped in two deploy attempts — see `KNOWN_TECHNICAL_DEBT.md`'s two 2026-07-19 postmortem rows for the production incident and its recurrence.

**Real SMTP delivery (shipped 2026-07-19):** production ran on the no-op provider (fabricated success, nothing actually sent) from Slice C through the first beta feedback round — root-caused after testers reported never receiving verification emails, see `KNOWN_TECHNICAL_DEBT.md`. Code is now ready for real delivery via Gmail SMTP, entirely environment-driven (`EMAIL_PROVIDER_KEY`, `SMTP_HOST`/`SMTP_PORT`/`SMTP_USERNAME`/`SMTP_PASSWORD`, `EMAIL_DEFAULT_SENDER`, `EMAIL_SENDER_DISPLAY_NAME` — see `DEVELOPMENT_WORKFLOW.md`'s Email Delivery section for the full list and Gmail App Password setup) — actual activation depends on those values being set in the Render dashboard, a manual step outside version control. `api/main.py`'s `_enforce_startup_guardrails()` now refuses to boot in production if the resolved provider is still the no-op one, matching the existing fail-closed pattern for `DEBUG`/`BACKEND_SECRET_KEY`, rather than silently running with email disabled again.

**Automatic participant claiming (backend infrastructure only, shipped 2026-07-19 — Slice D):** `api/services/participant_claiming.py::claim_participants_for_user(db, user)` runs as part of the same transaction as `verify_email_token()` (no intermediate commit — token validation, `email_verified_at`, claiming, and audit events all roll back together on any failure) — it finds every `Participant` row with `user_id IS NULL` whose email normalizes (via the shared `api/utils/email_normalization.py::normalize_email()`) to the same value as the verified user's email, links it (`participant.user_id = user.id`), and records one `AdminAuditEvent` per claim (`domain="participants"`, `action="participant_account_claimed"`, `actor_user_id` = the verifying user). Naturally idempotent — claimed rows are excluded from any future match by the `user_id IS NULL` filter itself, no separate bookkeeping needed. `POST /auth/verify-email/confirm`'s success response now includes `claimed_registrations: <count>`. Never touches already-linked `Participant` rows regardless of email match.

**Verification & claiming UI (shipped 2026-07-19 — Slice E, the last piece of the Hybrid Participant Account Model roadmap):** `/portal/verify-email` (`PortalVerifyEmail.jsx`) is a single page rendering one of several states depending on outcome — verifying, verified (with claimed-registration count), expired, already used, invalid, or (no `?token=` present) a check-your-email/resend prompt — reusing the loading/error/success pattern `PortalRegister.jsx` already established rather than introducing a new one. A `PortalVerificationBanner` (new, in `PortalLayout.jsx`) shows a one-click resend for any signed-in participant whose stored profile has a falsy `email_verified_at`, and clears itself automatically once verified. Two small, additive backend touches made this possible without any new route, schema change, or migration: `UserResponse` now includes `email_verified_at` (the column already existed; `GET /auth/me` just didn't expose it), and the verification email now links to `{PUBLIC_WEB_ORIGIN}/sfa-admin-testing/portal/verify-email?token=...` instead of a raw token (`PUBLIC_WEB_ORIGIN` already existed for CORS, `/sfa-admin-testing/` already existed in `vite.config.js`'s `base`). `portalAuth.js` gained `refreshPortalProfile()` so a successful verification updates the stored session immediately, without a re-login, so the banner disappears in the same client-side render. `PortalMyRegistrations.jsx` shows a one-time welcome banner (via router `state`, not a URL param, so a refresh doesn't repeat it) when arriving from a verification that claimed one or more historical registrations. Deliberately excludes manual claiming, account management, and conflict-resolution UI — none of those are needed given claiming is fully automatic.

| Layer | Location |
|---|---|
| Backend router | `api/routers/auth.py` |
| Backend service/support | `api/dependencies.py`, `api/security.py`, `api/services/authorization.py`, `api/services/rate_limiting.py`, `api/utils/email_normalization.py`, `api/services/account_verification.py`, `api/services/email_delivery.py`, `api/services/participant_claiming.py`, `api/services/admin_audit.py` |
| Data model | `User` (incl. `email_verified_at`, now exposed via `UserResponse`), `UserActionToken` (migration `e8b4a2f6c1d9`), `Participant.user_id` (claim target), `AdminAuditEvent` (claim audit trail, no schema change — `action` is a free-form column) |
| Frontend | `admin-app/src/pages/Login.jsx`, `admin-app/src/api/auth.js` (admin shell); `admin-app/src/pages/PortalVerifyEmail.jsx`, `admin-app/src/components/PortalVerificationBanner.jsx` (participant portal, see §14) |
...
**Feature Status:** Hybrid Participant Account Model roadmap complete (Slices A–E). A public, unauthenticated route group (`/portal`, `/portal/events`, `/portal/register`, `/portal/my-registrations`, `/portal/login`, `/portal/create-account`, `/portal/verify-email`) with its own layout, branding, and navigation, separate from the admin SPA shell. Account creation, email verification (with automatic historical-registration claiming), and profile management via `/auth/me` all exist; family/guardian accounts remain deliberately out of scope per the architecture review (§9 below). Anything beyond a read-only registrations view — registration editing, cancellation, badges, progress tracking, timeline/history — is still deferred, with no slice currently planned. `App()` (`admin-app/src/App.jsx`) checks `location.pathname` for the `/portal` prefix and, if matched, renders an entirely separate `<Routes>` tree wrapped in `PortalLayout` instead of the admin tree wrapped in `AppLayout` — the two trees never interact, so this is reachable with no token and cannot affect (or be affected by) the existing `if (!token) redirect to /login` admin gating. `PortalEvents` consumes the existing public `GET /api/events` endpoint (§2) for display only.
...
**Account creation (`/portal/create-account`, shipped 2026-07-19 — Hybrid Participant Account Model roadmap, Slice B):** `PortalCreateAccount.jsx` reuses the Slice A-hardened `POST /auth/register` (via `api/portalAuth.js::createParticipantAccount()`) and immediately chains into the existing, unmodified `loginParticipant()` (`registerAndSignIn()`) — account creation, sign-in, and redirect to `/portal/my-registrations` in one flow, no new session logic. A real visitor can now obtain a participant account end-to-end for the first time. `POST /auth/register` now also auto-issues an email verification token as of Slice C, and verifying that token now automatically claims any matching historical registrations as of Slice D (§1) — both transparent to this flow, since neither gates login or registration. The verification/claiming experience itself (`/portal/verify-email`, the resend banner) shipped as Slice E — see §1 — completing the Hybrid Participant Account Model roadmap.
